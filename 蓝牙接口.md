...
<br/>
<h2 id="connect">一、连接设备</h2>
符合MiService协议的设备要走安全连接，如果走普通连接会超时自动断开。

<h3 id="connect1">普通连接</h3>
```Java
...
<h3 id="connect2">MiService安全连接</h3>
...
如果安全连接返回的code为TOKEN_NOT_MATCHED，则调用以下接口清除本地token，ApiLevel为31
```
...
<h3 id="longconn">长连接<h3>

**维持长连接**

只在MIUI上支持，注意ApiLevel要不小于21。如果连接失败，则会隔一段时间尝试重连，如果继续失败，则重连间隔会翻倍，直到上限。

```
void bindDevice(String mac);
```

**解除长连接**

```
void unBindDevice(String mac);
```
...
如果希望蓝牙写时带WRITE_TYPE_NO_RESPONSE标志，则用如下接口，ApiLevel为31
```
...
## 七、固件升级
### （一）、升级信息查询
回调的BtFirmwareUpdateInfo中包括最新蓝牙固件信息，包括最新固件的版本、url、changeLog和md5。
```Java
...
后来新增下载固件接口可获取文件md5，如下，该接口APILevel为32.
...
插件中设置页中重命名后会发送广播通知结果，插件可监听如下广播：
