...
public class KeyManagementActivity extends XmPluginBaseActivity implements View.OnClickListener {
    ...
    private Button mGetBindInfoButton;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mGetKeyButton = (Button) findViewById(R.id.get_key);
        mGetBindInfoButton = (Button) findViewById(R.id.get_bind_info);
        ...
        mGetKeyButton.setOnClickListener(this);
        mGetBindInfoButton.setOnClickListener(this);
    ...
    public void onClick(View v) {
        if (v == mShareButton) {
            ...
            if (!TextUtils.isEmpty(shareUid)) {
                ...
                // activeTime/expireTime，所有与分享有关的时间戳传输对象均为 UTC 时间
                // 传输的经常型授权生效时间及失效时间以当天对应时间生成时间戳,如设置为 9:00 到 10:00,则时间戳为当天的 9:00 与 10:00 对应的 UTC 时间戳,跨 0 点的结束时间取当天第二日对应时间
                // 由于加密算法限制,时间戳超过 unsign int32(4294967295)的时间戳认为无效
                long activeTime = getUTCTime(); // 生效时间 UTC时间戳，单位为s
                long expireTime = activeTime + 1 * DateUtils.YEAR_IN_MILLIS; // 过期时间 UTC时间戳，单位为s
                List<Integer> weekdays = null; // 生效日期（星期几，例如周一和周三对应1和3，[1, 3]），仅在status=2时不可为空，周日用0表示
                boolean readonly = false; // true：被分享人不可接收锁push，false：被分享人可接收锁push
                // 分享钥匙的有效时间可以配置
        ...
        } else if (v == mGetKeyButton) {
            getSecurityKey();
        } else if (v == mGetBindInfoButton) {
            getBindInfo();
    ...
    public void getBindInfo() {
        XmPluginHostApi.instance().getBleLockBindInfo(mDevice.getModel(), mDevice.getDid(), new Callback<String>() {
            @Override
            public void onSuccess(String s) {
                updateContent("绑定时间: " + s);
            }

            @Override
            public void onFailure(int i, String s) {
                updateContent("获取绑定时间失败, code = " + i + ", detail = " + s);
            }
        });
    }
}
